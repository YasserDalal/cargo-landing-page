  ...
  return (
    <div className={className}>
      <div className='flex flex-col gap-2 pb-8'>
        <h2 className={`text-[clamp(22px,2.43vw,30px)] max-[920px]:text-[clamp(20px,5vw,30px)] font-montserrat font-bold text-[#F3F4F6FF] ${language.arabic && 'text-right'}`}>
          {language.arabic ? 'تفاصيلنا' : 'Our Details'}
        </h2>
        <h3 className={`text-[#BDC1CA] text-[clamp(14px,3vw,16px)] leading-6 ${language.arabic && 'text-right'}`}>
          {language.arabic ? 'تواصل معنا مباشرة من خلال القنوات التالية.' : 'Reach out to us directly through the following channels.'}
        </h3>
      </div>
      <div className='flex flex-col gap-6 text-[#F3F4F6] text-[clamp(14px,3vw,16px)]'>
        <div className={`flex gap-4 ${language.arabic && 'flex-row-reverse text-right'}`}>
          <div className='w-5'>
        ...
        <div className={`flex gap-4 items-center ${language.arabic && 'flex-row-reverse text-right'}`}>
          <div className='w-5'>
