    ...
    <div className={className}>
      <h2>{
        language.english 
          ...
          : language.arabic && 'احصل على عرض سعر واتصل بنا'
      }</h2>
    </div>
  );
}
...
function Contents({ children }) {
  const { language } = useLanguage();
  return (
    <div className={`flex ${language.arabic && 'flex-row-reverse'} gap-12 max-[920px]:flex-col max-[920px]:items-center max-[1050px]:gap-6 w-full justify-center`}>
      {children}
    </div>
  );
}
